...
if ( report == 1 )
    ...
    set(gca,'Fontsize',10,'xlim',xlimplot,'ylim',ylimplot,'linewidth',1.0);
    axis ij;
    axis image;
    axis off;
    ...
    whitebg
    plot(leavesout(leafind,5),'-b','markeredgecolor','b','linewidth',1.0)
    hold on
    plot(leavesout(leafind,6),'--r','markeredgecolor','b','linewidth',1.0);
    hold off
    grid on
    line([1 length(leafind)],[0 0],'color','k','linewidth',1.0);
    set(gca,'xlim',[1 length(leafind)],'xtick',[1:1:length(leafind)],'Fontsize',10,'linewidth',1)
    pbaspect([1 0.7 1]);
    
    legend('Pos Leaf','Neg Leaf')
    xlabel('Leaf Pair','Fontsize',12);
